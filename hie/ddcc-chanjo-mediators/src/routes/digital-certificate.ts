...
router.post(
  ...
  async (request, response) => {
    try {
      ...
      //Loop through
      const immunizations = [];
      Object.entries(_vaccineCodes).forEach(async ([code, display]) => {
        const immunization = {
          ...
          vaccineCode: {
            coding: [
              {
                code: code,
                display: `IMPO-${display}`, // Adjust the display name as needed
              },
            ],
            text: display,
          },
          patient: {
            reference: "Patient/d1c0ca77-d466-4984-ba1f-12a8271c9e0b", // Replace with actual patient reference
          },
...
router.post("/nice", async (req, res) => {
  try {
    ...
    savePDFToFileSystem(
      pdfFile,
      `${patientId}-${vaccineName}.pdf`.replace("/", "-")
    );
