...
def create_date_query(start_date=None, end_date=None):
    search_query = ""
    if start_date:
        date_query = (datetime.strptime(start_date, '%Y-%m-%d') - timedelta(days=14)).strftime('%Y-%m-%d')
        search_query += f"AND Due_Date >= '{date_query}' "
    if end_date:
        date_query = (datetime.strptime(end_date, '%Y-%m-%d') - timedelta(days=14)).strftime('%Y-%m-%d')
        search_query += f"AND Due_Date <= '{date_query}' "
...
def insert_data(data):
    ...
    INSERT INTO primary_immunization_dataset (
        patient_id, family_name, given_name, national_id, gender, phone,
...
def query_defaulters(name='', vaccine_name='', start_date='', end_date=''):
    search_query = ""
    if name:
        search_query += f"AND (family_name ILIKE '%{name}%' OR given_name ILIKE '%{name}%') "
    if vaccine_name:
...
print("Table 'primary_immunization_dataset' created successfully (if it didn't already exist).")
